    ...
    <div className="bg-secondary text-secondary-foreground py-2.5 px-4 text-sm hidden md:block relative">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        ...
        <div className="flex items-center space-x-6">
          {socialJson.socialLinks.find(link => link.name === "LinkedIn") && (
            <a
              href={socialJson.socialLinks.find(link => link.name === "LinkedIn")?.url}
              target="_blank"
          ...
          {socialJson.socialLinks.find(link => link.name === "GitHub") && (
            <a
              href={socialJson.socialLinks.find(link => link.name === "GitHub")?.url}
              target="_blank"
              rel="noopener noreferrer"
              className="hover:text-primary transition-colors flex items-center gap-2"
            >
